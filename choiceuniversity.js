(function(root, factory) {
  ...
  var loadDependence = function(fncallback) {
    ...
    if (!window.wwload.choiceuniversity) {
      window.wwload.choiceuniversity = "wait";
      requirejs.config({
        paths: {
          "school": "libs/choiceuniversity/js/school",
          //"choiceuniversity": "libs/choiceuniversity/js/university" // 示例: libs/qrcodejs/qrcode
        //  "choiceuniversity-css": "libs/choiceuniversity/css/university"
        }
      });
      require(["school","css!libs/choiceuniversity/css/university"], function(provinceArray,provicneSelectStr) {
        window.wwload.choiceuniversity = true;
        window.provinceArray=provinceArray;
        window.provicneSelectStr=provicneSelectStr;
        replace();
        fncallback();
      });
    } else if (window.wwload.choiceuniversity === "wait") {
      setTimeout(function () {
...
//console.log(province);
  ...
  var init = function() {
    ...
    $("body").append('<div id="proSchool" class="provinceSchool" style="background: #fff;"><div class="title"><span>请选择学校</span></div><div class="proSelect"><select></select><span>如没找到选择项，请选择其他手动填写</span><input type="text" /></div><div class="schoolList"><ul></ul></div><div class="button"><button flag="0">取消</button><button flag="1">确定</button></div></div>');
    //$("bady").append('<style>provinceSchool{background:#fff;}</style>')
   ...
   */
   function school1($ele){
      //province;
      //proSchool;
      //学校名称 激活状态
      $("#schoolName").focus(function(){
        var top = $(this).position().top+$(this).height()+22;
      ...
      $(".schoolList").on("click","li",function(){
        var data = $("#schoolName").val($(this).html());
      ...
      $(".button").on("click","button",function(){
        ...
        var selectPro = $("div[class='proSelect'] select").val();
        if("99"==selectPro){
          var data = $("#schoolName").val($("div[class='proSelect'] input").val());
        }
        $("div[class='provinceSchool']").hide();
        }
      });
  ...
  function processElement($ele) {
    ...
    $.wwclass.syslog(message,category,severity,from,opt);
    //*/
    var selected = $.wwclass.helper.getJSONprop($ele, "data--svalue") || [];
    school1($ele);
